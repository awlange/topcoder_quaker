...
class Network:
    """
    Our multi-layer perceptron neural network
    """
    ...
    def set_random_biases_and_weights(self):
        self.biases = [[random.gauss(0.0, 1.0) for _ in range(layer_size)] for layer_size in self.sizes[1:]]
        self.weights = []
        for prev_layer_size, layer_size in zip(self.sizes[:-1], self.sizes[1:]):
            tmp = []
            for i in range(layer_size):
                tmp.append([random.gauss(0.0, 1.0) for _ in range(prev_layer_size)])
