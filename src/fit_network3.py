...
class QuadraticCost:
    ...
    @staticmethod
    def delta_regression(z, a, y, rw):
        """Return the error delta from the output layer."""
        sz = sigmoid_prime_vec(z)
        rws = np.dot(rw, sz)
        res = (a-y) * rws
        return res

    @staticmethod
    def delta_rw(amin1, a, y):
        # delta for the regression layer w/r w
        # return (a-y) * sigmoid_prime_vec(z)
        # return (a-y) * d(rw.sz) = (a-y) * d(rw).sz
        return (a-y) * amin1

    @staticmethod
    def delta_rb(a, y):
        # delta for the regression layer w/r b
        # return (a-y) * sigmoid_prime_vec(z)
        return a-y
...
class Network:
    ...
    def __init__(self, sizes, cost=QuadraticCost):
        ...
        Plus we've added a regression layer on the end.
        """
    ...
    def default_weight_initializer(self):
        ...
        # self.regression_bias = np.random.randn(1, 1)
        self.regression_weights = np.random.randn(1, self.sizes[-1]) * 100

    def feedforward(self, a):
        """
        Return the output of the network if ``a`` is input.
        Does not include the regression layer!
        """
        for b, w in zip(self.biases, self.weights):
            a = sigmoid_vec(np.dot(w, a)+b)
        return a

    def feedforward_with_regression(self, a):
        """
        Return the output of the network if ``a`` is input.
        Does not include the regression layer!
        """
        ...
        # Use the activation for the regression layer
        # reg = np.dot(self.regression_weights, a) + self.regression_bias
        reg = np.dot(self.regression_weights, a)
    ...
    def update_mini_batch(self, mini_batch, eta, lmbda, n):
        ...
        # Regression layer update
        # nabla_rb = np.zeros((1, 1))
        nabla_rw = [np.zeros(w.shape) for w in self.regression_weights]
        for x, y in mini_batch:
            activation = self.feedforward(x)
            regression = self.feedforward_with_regression(x)
            # delta_nabla_rb = self.cost.delta_rb(regression, y)
            delta_nabla_rw = self.cost.delta_rw(activation, regression, y)
            # nabla_rb = [nb+dnb for nb, dnb in zip(nabla_rb, delta_nabla_rb)]
            nabla_rw = [nw+dnw for nw, dnw in zip(nabla_rw, delta_nabla_rw)]

        m = np.zeros((1, 1))
        m[0] = eta/len(mini_batch)

        # self.regression_bias = self.regression_bias - m*nabla_rb
        rw = [w-(eta/len(mini_batch))*nw for w, nw in zip(self.regression_weights, nabla_rw)]
        # self.regression_weights = rw
    ...
    def backprop(self, x, y):
        ...
        regression = self.feedforward_with_regression(x)
        # z = np.dot(self.regression_weights, activation) + self.regression_bias
        z = np.dot(self.regression_weights, activation)
        # backward pass
        delta = self.cost.delta_regression(activations[-1], regression, y, self.regression_weights)
        delta = self.cost.delta(zs[-1], activations[-1], y) * delta
        nabla_b[-1] = delta
    ...
    def total_cost(self, data, lmbda, convert=False):
        ...
        for x, y in data:
            ...
            a = self.feedforward_with_regression(x)
            if convert:
...
def divide_raw_data(raw_data):
    ...
    for x, y in raw_data:
        if y > 0.0:
            positive_data.append((x, vectorized_result(1)))
        else:
            negative_data.append((x, vectorized_result(0)))
...
def main():
    ...
    for bootstrap in [3, 4, 5]:
        try:
            print("Bootstrap: {}".format(str(bootstrap)))

            train_positive_data, test_positive_data = split_data(positive_data, 0.7)
            train_negative_data, test_negative_data = split_data(negative_data, 0.4)  # Don't need all those negatives
            ...
            evaluation_cost, evaluation_accuracy, training_cost, training_accuracy = network.SGD(
                train_data, n_epochs, len(train_data) // 10, 0.05, 0.001,
            ...
            with open("output/accuracy_" + str(bootstrap) + ".dat", "w") as fa:
                accuracy_train = network.accuracy(train_data)
                accuracy_test = network.accuracy(test_data)
...
def main2():
        ...
        for line in f:
            ...
            if n_line > 1:
                ...
                raw_data.append((np.reshape(split_line[2:], (len(split_line[2:]), 1)),
                                 vectorized_result4(split_line[1])))
    ...
    # Start the fit
    network.SGD(raw_data, n_epochs, 2, 0.05, monitor_training_cost=True)

    for x, y in raw_data:
        result = network.feedforward_with_regression(x)
        print(result, y)
...
def test_network2():
    ...
    act = [2.0, 3.0]
    result = network.feedforward_with_regression(np.reshape(act, (2, 1)))
    print(result)

    data = []
    data.append((np.reshape([2.0, 3.0], (2, 1)), vectorized_result4(7.0)))
    data.append((np.reshape([4.0, 6.0], (2, 1)), vectorized_result4(25.0)))
    data.append((np.reshape([5.0, 5.0], (2, 1)), vectorized_result4(25.0)))
    data.append((np.reshape([1.0, 4.0], (2, 1)), vectorized_result4(5.0)))

    network.SGD(data, 50, 1, 0.01, monitor_training_cost=True)

    for x, y in data:
        act = network.feedforward(x)
        result = network.feedforward_with_regression(x)
        print(act, result, y)
